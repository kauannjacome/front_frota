    ...
    async function load() {
      try {
        ...
        const formValues: Partial<TripFormValues> = {
          ...
          start_time: data.journey_start ? moment(data.journey_start) : undefined,
          back_time: data.journey_back ? moment(data.journey_back) : undefined,
  ...
  const handleFinish = async (values: TripFormValues, persons: PersonRow[]) => {
    ...
    try {
      ...
      const payload = {
        ...
        journey_start: values.start_time?.toISOString() ?? null,
        journey_back: values.back_time?.toISOString() ?? null,
